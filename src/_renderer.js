...
function initGraphicalErrorHandling() {
    ...
    window.onerror = (msg, path, line, col, error) => {
        ...
        window.LycanErrorsModals.push(errorModal);
...
function displayLine() {
    ...
    switch(true) {
        case i === 2:
            bootScreen.innerHTML += `Lycan-UI Kernel version ${electron.remote.app.getVersion()} boot at ${Date().toString()}; root:xnu-1699.22.73~1/RELEASE_X86_64`;
        case i === 4:
...
async function displayTitleScreen() {
    ...
    bootScreen.innerHTML = "<h1>Lycan-UI</h1>";
...
const { OllamaAIInterface } = require("./classes/ollamaAIInterface.class");
const { BasicTextEditor } = require("./classes/basicTextEditor.class");
...
window.mods.ollamaAI = new OllamaAIInterface("mod_column_left");
...
window.mods.textEditor = new BasicTextEditor("mod_column_right");
...
window.openSettings = async () => {
    ...
    new Modal({
        ...
        buttons: [
            ...
            {label: "Restart Lycan", action: "electron.remote.app.relaunch();electron.remote.app.quit();"}
        ]
    }, () => {
